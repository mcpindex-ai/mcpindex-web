...
<!--
  ...
  security operator screenshots. The render pipeline applies the brand palette to them (Geist Mono,
  ...
  quotable standfirsts (the Monday/Tuesday press line + the gate/network operator line) - both
...
<!-- TITLE (Geist Sans, Ink). RENDER HIERARCHY (M94, third-round-running - LAND IT IN THE RENDER, not just copy):
     ...
     ONE operator couplet → version; the Abstract / Exec-summary / operator-fast-path follow the cover, in that order. -->
...
⬡ **The trust-to-act layer: it pins the contract of every tool you wire through it and holds the call the moment that contract moves.**
...
<!-- Below the rule: the subordinated cover material - the operator/investor line, the honesty beat, the version.
...
**mcpindex is the trust-to-act layer for agent tool calls: it pins the contract of every tool you wire through it and holds the call the moment that contract moves.** Under the Model Context Protocol, the tool your agent trusted can change its contract between one call and the next - silently, with no version bump, from a server you do not run - and your agent acts on the change as if nothing moved. No host enforces a persistent contract baseline in-path today, and the protocol does not require one, so nothing in the standard publish-time stack catches that day-two silent change. That is the seam mcpindex closes, and the bet is bigger than the catch.

The day-one deliverable is the **local tier-0 gate**: the diff itself makes no network call, standalone, complete on your host the day you install it, on the persistent stdio/SDK path - the value lands the first time a contract moves, not at install. The part that compounds, and the moat, is the layer the gate grows into: the cross-install verdict corpus, the outcome-flywheel calibration *it will accrue*, and the governance an ecosystem embeds against - recognized across every install rather than re-derived on each (§6). The gate is live today; the network accrues. The gate earns the install; the network keeps it.

### ⬡ Executive summary (read this if you read nothing else)

- **The wedge.** A free, source-available, local gate that pins each tool's contract and HOLDs the call the moment the contract moves, on your host, the day you install it. A credential-blind receipt per gated call is on by default and one env var turns it off.
...
- **The ask.** Install the gate; contribute to the network.

> **⬡ Reader-router.** **Product / operator / press path (the ~7-page spine):** §1 problem → §3 the two surfaces → §4 architecture → §7 the proof → §6 the moat → §9 the limits → §11. **Security review path:** §2 (the honest box) → §8 (deployment, trust boundary) → the Appendix (full adversary perimeter + questionnaire-grade math). Read the spine for the decision; read the security band for the audit.
...
> - **What "no banner" means:** *protected and quiet*. Day one the gate pins every tool it is wired in front of and watches; the first banner fires the first time a contract moves, not at install. The first-10-minutes walkthrough (fire a HOLD yourself with `mcpindex-gate`, read the pinned-inventory card) is §8.6.
> - **Free for the gate:** unlimited local gating, all postures, every host - free under the stated license. A credential-blind per-call receipt is on by default; `MCPINDEX_RECEIPT_INGEST_ENABLED=0` suppresses it. The cross-install network is free too, and opt-in.
...
> **⬡ This begins the security-review band.** The narrative spine is §1 → §3 → §4 → §7 → §6 → §9 → §11; a product, operator, or press reader gets the honest box around the win **in §4** (right after the mechanism is shown) and can skip straight to §3. This section, §2.5, and §8.1-§8.5 are the audit-grade perimeter for a security reviewer - read in full when you are doing the threat-model assessment, not the product evaluation.
...
The whole product shape on one screen: two surfaces, four tiers, five deployment forms, marked live-at-launch vs. held. The single read before the architecture prose; the box a operator screenshots to explain mcpindex internally. **Read the "Live at launch" column literally:** what is genuinely live day-one is the tier-0 gate, tier-3-when-you-wire-a-verifier, and the five deploy forms - tiers 1 and 2 are built-but-held *activating* seams, not load-bearing for the day-one value. "Full platform" means the mechanism is all shipped and audited; the *populated* network accrues, it is not flipped on (§6 cold-start).
...
- **Monitor**: releases every drift hold, though tamper evidence and an internal error sit above the posture layer and still stop the call. Every drift PROCEEDs; a safety-relevant drift is recorded and surfaced as a notify-only note. The break-glass posture for an outage, and the recommended first-week posture for sizing your own HOLD rate before switching to Guard.
...
**Free and open.** The in-path gate is MIT (`mcpindex-gate` on PyPI). The TypeScript SDK and this site are PolyForm-Noncommercial-1.0.0. The diff method is published here.
...
> **⬡ Enterprise-readiness matrix: built / flag-on / operator-provisioned.** **Built and on by default:** the local tier-0 gate, all postures, the SDK, the stdio proxy with persistent pins (the gateway pin is per-process today, §4.2), the local telemetry/error sink (local capture on; egress plane held/off by default), the in-process tenant-isolation primitives. **Flag-gated (ships off; single-tenant is the launch default):** multi-tenant mode (`MCPINDEX_GATEWAY_MULTI_TENANT`). **Buyer-provisioned (held seams, §10):** the cloud tier-1 corpus, the tier-2 LLM consult, real OIDC/JWKS tenant identity, the KMS-backed signer, the durably-chained SIEM tenant log, and multi-region HA.
...
So a operator reads the *proxy* boundary as persistent-pin + subprocess upstream (no SSRF, no auth-thread), and the *gateway* boundary as ephemeral-pin + loopback-bind + SSRF-gated/auth-threaded HTTPS upstream - never the two fused. (Render note: both boundary figures carry the ⬡ glyph on the gate node and amber on the `definition_hash ONLY` egress label only; the most-screenshotted artifacts, branded.)
...
| `{definition_hash, verdict, reason_class}` | contribute path (tier-1) | **OFF** | name-free triple; closed-vocab `reason_class` |
...
| name-free outcome aggregate / scrubbed error breadcrumb | telemetry / error sink | **OFF** | local capture is always-on, zero-egress; the egress plane defaults to `HeldEmitter` (no-op); `DO_NOT_TRACK`, an operator opt-out, or one-click-off hard-veto egress |
...
**What a single-tenant operator gets at launch.** The single-tenant launch default (what §8.2 says a operator deploys) gets a **computed on-device 30-day local stats card** (`cse/stats.py` `LocalStats`, fed by *both* the proxy and the SDK via `gate_stats.record_gate_call`, zero-egress): calls checked, HOLDs, re-pins/overrides, tier escalations, added latency, month-over-month catches, plus an on-host JSONL error/telemetry sink and an on-device outcomes store. So "how do I monitor gate HOLDs - and how noisy is Guard on my traffic" is answered by *a computed local card*, not raw log-grepping. There is **no mcpindex-operated fleet aggregation, no SIEM push** at launch; the decisions land in a local structured log each host's own agent can forward. A durable, exportable, multi-tenant decision feed a SOC ingests directly is the enterprise §10 SIEM seam, not the launch default.

**Data residency, retention, sub-processors.** All stores are on-host today: pins (`~/.mcpindex/pins/`), the local verdict cache, the per-tenant audit ledger, and the local telemetry/error capture (`~/Library/Logs/mcpindex-errors.jsonl`). Pins persist until re-pinned; the audit ledger and the local error log are bounded (ring-buffer / size-capped rotation); retention is configurable by the operator. **No operator data reaches any sub-processor in the default (held) configuration.** When an operator enables the *canonical mcpindex* cloud seam, tier-1 runs on **Vercel + Supabase (US-region at launch; EU-residency is a §10 item)** as named sub-processors, provisioned in lockstep with seam activation (§5.2); the only data that transits is the `definition_hash`, and the sub-processor set is disclosed here rather than in a contract, because no contract is on offer.

**Cloud-corpus at-rest posture (the Network tier - distinct from the on-host "N/A").** The on-host "encryption at rest: N/A" answer holds only for the default configuration, where stores carry no secrets. The *moment* the cloud seam is enabled, `definition_hash` values land in Supabase: **Supabase-managed AES-256 at rest**, and the corpus holds **only one-way `definition_hash`es plus closed-vocabulary verdict/reason-class data - no arguments, no schema text, no credentials, no tenant-segregated PII**. Whether corpus rows are tenant-attributable, the key-custody model for the cloud backend, and any tenant-segregation guarantee are **documented before the seam can be enabled** - the on-host "N/A" does not stand in for the cloud case, and an operator evaluating the cloud tier reads this row, not the on-host one.

**Compliance / data-handling posture.** In the default configuration one thing leaves the host: a credential-blind receipt per gated call, carrying a contract hash, a verdict, closed-vocabulary run and outcome fields and an hour-rounded timestamp, never arguments, results, server names or URLs. `MCPINDEX_RECEIPT_INGEST_ENABLED=0` suppresses it entirely. No customer or personal data is in that receipt. When the cloud seam is enabled, the only datum that transits is a one-way SHA over a *public-contract field-set*, not personal data, not customer content, which keeps the GDPR/CCPA surface minimal; the sub-processor disclosure above is the whole of the commitment; there is no contract behind it.

**For an EU operator specifically.** At launch the default posture is **on-host, zero cross-border transfer** - no datum leaves the EU host because no datum leaves the host at all. The only state in which anything transits is an operator-enabled cloud seam, and then the single datum is the one-way `definition_hash` to a **US-region** sub-processor set (Vercel + Supabase), with no contractual transfer mechanism behind it, because there is no contract on offer; the contract over which the hash is computed never leaves. **EU data residency for the cloud seam is a named §10 item** - until it lands, an EU operator with a hard in-region requirement should run on-host only (the launch default), which transfers nothing.
...
- **Host plugin / one-click install.** Two install paths ship and execute as written:
  ...
  > **⬡ Locked-down / no-rewrite install path (the manual variant).** A operator whose endpoint-security policy forbids config rewrites or resident daemons can install **proxy-only, with no host-config rewrite and no background watcher** - *without* running `install.sh` at all:
  >
  > ```bash
...
**Single-writer on the write plane; stateless on the read plane.** Split by plane, because the two scale differently: the **read/serve plane is stateless and horizontally scalable by design** (`enterprise/serving.py` - identical stateless nodes behind the edge), so verdict reads N-node out. The **write plane - the append-only `DriftCorpus` chain plus the verdict/pin stores - is single-writer** (SQLite WAL): one host, one writer; the append-only chain cannot be fanned across concurrent writers. So enterprise HA is **active-passive failover on the *writer*, not multi-writer scale-out**: held decisions are fail-closed during a failover (no positive clearance minted while degraded), and SLA/RTO is operator-negotiated (see §8.5 for the default target).

### 8.3 Security assurance

mcpindex is pre-SOC-2 and carries no third-party security assessment. What stands in its place is auditability: **the in-path client is source-available and import-firewalled (`open_core_manifest.py`, §6 governance axis), so you can read exactly what runs on your host**, and the §7 dogfood is reproducible evidence of what the gate does. Available *now* for review: the in-path client source + the import firewall, the reproducible §7 dogfood, and the published SBOM + `security.txt`. No third-party pen test, no SOC 2 (§10). That is stated here rather than discovered mid-review.

**Control-framework posture.** The architecture is **designed toward the SOC 2 TSC Security + Confidentiality criteria** (zero credential custody, closed-vocabulary stores, fail-closed boundaries) - designed-toward, not attested. On AI governance: the verdict is **advisory and human-in-the-loop by design** (a HOLD routes to a human; the gate mints no autonomous clearance, §5.5), which is the relevant posture for an EU AI Act / NIST AI RMF classification - mcpindex is a change-detector that defers the decision to a person, not an automated decision system. None of this substitutes for third-party attestation; it gives a reviewer the map.

**The as-is / no-warranty asymmetry, named.** The assurance model is **operator-led code audit plus the reproducible §7 proof**. Read that plainly: there is no warranty, no third-party attestation, and no commercial engagement standing behind this software. If your risk process requires contractual risk-transfer, this project does not offer it, and no amount of auditability substitutes for it. Naming the asymmetry builds more trust than implying an audit covers it.

**Install integrity & supply chain.** The install artifact is the PyPI package **`mcpindex-gate`**, which is **hash-pinnable** (pin the version + hash in your own lockfile/policy). Each release publishes a **checksum/signature** alongside `security.txt`; SLSA/sigstore build provenance is a named §10 item, not implied as present. On macOS, the install artifacts are **notarized / code-signed at launch** (so Gatekeeper does not block them); the resident config-rewriting watcher may need an **EDR allow-list entry** on a managed fleet, and the manual no-rewrite/no-watcher path (§8.2) is the EDR-friendly default for that case. The resident watcher runs at **user level** (LaunchAgent / systemd user unit, **not root**, §8.2). For a locked-down endpoint, the **manual no-rewrite install path** (§8.2: `uv tool install mcpindex-gate` + `config_wire`, no `install.sh`) installs the proxy with **no host-config rewrite and no background watcher** - the install variant most likely to clear an endpoint-security gate. The honest substitute for a not-yet-SOC-2 state is auditability: read the code that runs on your host, pin the artifact, and reproduce the proof.

**For a downstream security review today:** the in-path client is source-available and import-firewalled, so the forwardable artifacts are the repo itself and the reproducible §7 proof. There is no vendor to assess, because there is no vendor.

> **⬡ Security quick-reference (the questions, answered in one place).**
>
...
> | Control-framework posture | designed toward SOC 2 TSC Security + Confidentiality; not attested (§10). No self-assessment questionnaire is offered |
...
> | Vuln remediation | best-effort target: critical patch + advisory within 5 business days, high within 15. Not a contractual SLA; none is offered (§8.5) |
> | RTO / RPO | read plane stateless/N-node; write plane single-writer active-passive; best-effort only, no contractual SLA offered (§8.5) |
> | Encryption at rest | **On-host default:** N/A - stores carry no secrets (public contract hashes + public schemas only, §8.4); inherit the host file-permission model; pin store is tamper-EVIDENT on load, not integrity-checked: an unreadable or deleted store HOLDs, a coherent rewrite is not detected (§8.4 host-trust boundary). **Cloud seam (Network tier):** Supabase-managed AES-256; corpus holds only one-way `definition_hash`es + closed-vocab verdict data, no PII; tenant-attributability + key custody are documented before any seam is enabled (§8.1) |
...
> | Incident response / breach notify | §8.5: vendor-side compromise notification (72h target) **with named detection & containment** per vector (cloud corpus: manual contribute-anomaly review + seam-disable; supply chain: checksum-mismatch monitoring + yank/advisory), distinct from the (minimal) operator-data-breach surface |
> | Independent assurance | none. Source-available client + import firewall + reproducible §7 proof + SBOM/security.txt today; pen test + SOC 2 §10 |
> | SOC 2 | pre-SOC-2; Type 2 roadmap §10 |
> | CVD / disclosure | `security.txt` at launch; target first-response 2 business days |
> | Availability / RTO | read plane stateless/horizontally-scalable; write plane single-writer active-passive failover; best-effort, not contractual (§8.5) |
> | Nature of a verdict | advisory contract-diff, **not a warranty of safety** (§9 #1) |
>
> **No commercial offering.** mcpindex is a personal research project. There is no company selling it, no paid tier, no support contract, no DPA, and no negotiated liability, because there is nothing to negotiate with. The in-path client is provided **as-is, no warranty**, which is *why* the operating posture (route every HOLD to review; treat the gate as a change-tripwire) matters: mcpindex does not warrant that a PROCEED is safe or that a HOLD fired on every drift. Durable SIEM export and KMS-signed receipts are unbuilt (§10), not withheld pending a deal. The §9 limits are the whole of it.
...
- **Pin-store tamper model.** The pin store is plain on-host JSON under the user's home (`~/.mcpindex/pins/`), written atomically (mkstemp + `os.replace`). It inherits the host's file-permission model: a local process running **with the user's own privileges** can read or rewrite it, but such a process is already inside the §2 "compromised host" boundary. **As of gate 0.11.0 the pin store is TAMPER-EVIDENT, not tamper-proof.** A store that is present but unreadable - truncated, reshaped, a record type-flipped, replaced by a symlink, or deleted after having been pinned - is treated as EVIDENCE and HOLDs (`HoldClass.UNCHECKABLE`), in every posture and regardless of `fail_open`. It previously degraded to a cold store and silently re-TOFU'd whatever the server was serving at that moment, which made one file write a complete bypass. **What is still NOT detected: a COHERENT rewrite.** A same-privilege process that replaces the store with a well-formed one carrying its own baseline is trusted on next load, consistent with the §2 host-trust boundary - no same-user control prevents that, because any secret the gate can read, a process running as the user can read too. External anchoring (making a coherent rewrite detectable after the fact) is a candidate hardening, not a shipped feature. The store carries no secrets (only public contract hashes and captured public schemas) and is not a privilege-escalation path. The pin baseline is as trustworthy as the user account that owns it.
...
- **Dependency / SBOM posture.** The in-path client's defensible property is its small, auditable surface: stdlib plus a tight set of first-party modules and the MCP SDK, enforced by the public-surface import firewall (`open_core_manifest.py`, §6 governance axis). The in-path client's SBOM is published in a **standard machine-readable format (CycloneDX)** alongside `security.txt` - so a post-xz reviewer can ingest it directly and score the **single-digit-to-low-tens transitive-dependency count** rather than take "small auditable surface" on faith.

### 8.5 Operational posture and incident response

- **Operational targets (best-effort, stated not silent - and not contractual).** **Vuln remediation:** target a critical patch + advisory within **5 business days**, a high within **15**, on the CVD / `security.txt` channel. This is a maintainer's stated intent, not an SLA, and no SLA is on offer. **Availability:** the read/serve plane is stateless and horizontally scalable; the write plane is single-writer active-passive (§8.2), so the posture is **best-effort RTO/RPO** - usable because it is stated, not because it is strong. **Cloud-seam operational ownership:** while the canonical cloud tier-1 seam stays held (the default), there is no cloud operational surface to own at all.
...
- **Operator-data breach surface (minimal).** In the default configuration the only thing that leaves the host is a credential-blind per-call receipt (contract hash, verdict, closed-vocabulary fields, hour-rounded timestamp), suppressible with `MCPINDEX_RECEIPT_INGEST_ENABLED=0`. When the cloud seam is enabled, the only datum that transits is the `definition_hash`.
- **Project-side compromise notification (distinct from the above).** The operator-data surface is minimal, but mcpindex's own infrastructure can be compromised, which is a separate matter:
  - **Canonical cloud corpus compromise (Vercel + Supabase).** *Notification:* if the canonical cloud tier-1 corpus is breached, the stated intent is a public advisory within **72 hours**. Because the only datum the corpus holds is the one-way `definition_hash`, the operator-data exposure of such a breach is bounded by the non-invertibility of that hash (§8.1) - but the notification obligation stands regardless. *Detection & containment, stated honestly:* the detection control is **anomalous-contribute monitoring**, which today means **manual review of contribute volume and reason-class distribution**, not automated anomaly detection (a §10 maturity item); containment is **disable the seam + invalidate the affected `definition_hash` rows + advisory via `security.txt`**. The immaturity is named rather than papered over by the notification target.
  - **Supply-chain compromise of `mcpindex-gate`** (the in-path install artifact through which operator credentials transit). *Detection:* **signed-release verification + checksum-mismatch monitoring** - an operator who pins the version+hash (§8.3) is not silently upgraded onto a bad release, and a checksum mismatch against the published signature is the trip-wire. *Containment:* **yank the bad release + advisory via the CVD / `security.txt` channel** within the same target window. Hash-pinning is a *prevention* control; the detection-and-response capability is the checksum-mismatch trip-wire plus the yank/advisory path, stated so the notification target is backed by a detection story rather than only prevention. SLSA/sigstore build provenance (§10) strengthens detection further.
  - The **operator-data breach surface** (minimal, by design) is deliberately distinguished from the **project-compromise notification intent** (stated above) so a security reviewer is not left to infer the latter from the former.
...
- **Audit-retention disqualifier, in one place.** **If you carry a regulatory log-retention or tamper-evident-audit obligation, treat durable SIEM export (§10) as a gating prerequisite, not an enhancement: the launch tenant ledger is an in-memory ring buffer (evicts oldest-first, not durably hash-chained) - operational telemetry, not a retained audit record.** A regulated operator is not production-ready on the audit axis until the §10 SIEM export lands; we state this here rather than let it surface mid-evaluation.

---

## 9. Honest limits (stated as strength)

A trust product that over-claims its maturity loses the trust it is selling. **In practice the gate catches the change that actually bites you**: a silently-added required param, a destructive flip, a narrowed constraint, a reduced enum, a removed param, a rewritten description, after your agent already trusts the tool. The list below is the honest perimeter around that core, clustered into three groups. Each is tagged **intrinsic** (true at any version) or **at-v1** (maturity-gated, not a deploy flip). Each at-v1 limit names its §10 unblock so a operator can trace "this blocks me today → here is what unblocks it."

> **⬡ Read the list as confidence, not as a hedge.** Every limit below is one a competitor has and does not disclose. We disclose ours because a trust layer that hides its perimeter is not one. The §6 enterprise-readiness matrix maps built / flag-on / operator-provisioned in one place.
...
**C - What is built but operator-provisioned (at-v1).**
...
The §9 limits are not on this list as "coming soon"; they are intrinsic or genuinely unearned. What *is* on the roadmap is the maturity work and the provisioning work, grouped by horizon, by the gap each clears, and by the **trigger** that advances it (conditions, not dates - nothing here is a delivery promise).
...
A competitor can clone the diff classifier in a weekend. What it cannot clone is the **trust-to-act layer** - it pins the contract of every tool you wire through it and holds the call the moment that contract moves, and across installs that judgment is recognized everywhere rather than re-derived: the corpus of cross-install verdicts, the outcome-flywheel that turns catches into calibration, and the governance that enterprises and frameworks embed against. That is the defensible thing, and it is why the gate is only the wedge. In the dogfood, the wedge meant catching a silently-added `required` parameter (`owner`) mid-session: a held `-32010`, never forwarded (§7) - the whole product in one event.

The gate is deterministic, in-path, credential-storage-free, and honest about what its verdict is: a contract-diff, not a safety oracle. The honesty is not a hedge; it is the architecture. The provenance choke, the closed vocabularies, the fail-closed tiers, the held-by-default cloud and LLM seams, the `calibrated=false` shadow loop, the public-surface import firewall, and the OTS-anchored history all encode the discipline that a change detector must never masquerade as a safety oracle.
...
> - **Security reviewer:** the in-path client is source-available and import-firewalled - start with the repo and the reproducible §7 proof, then §8 for the trust boundary and the honest gaps. Questions: hello@mcpindex.ai.
