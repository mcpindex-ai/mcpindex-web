...
> uv tool install mcpindex-preflight        # the gate as a proxy/CLI — no host-config rewrite
...
- **Host plugin / one-click install.** Two install paths ship and execute as written:
  ...
  >
  > ```bash
  > uv tool install mcpindex-preflight          # installs the proxy artifact only — no rewrites, no watcher
...
> | Install integrity | `mcpindex-preflight` PyPI, hash-pinnable; checksum/signature at launch; SLSA §10 |
