...
- Method: a deterministic contract-diff (ChangeKind taxonomy: added-required-param, required-set-expanded, constraint-narrowed, type-changed, enum-values-removed, removed-param, annotation-flip-to-destructive, output-schema-added/changed, tool-added/removed), plus an injection/exfil marker scan over the input and output schema and description. Postures: Monitor (releases every drift hold; proceed-with-note; a tamper hold and an internal error still stop the call) / Guard (default; hold dangerous, INCONCLUSIVE where behaviour is the gate, auto-accept proven-benign) / Strict (hold anything not proven benign - NOT every drift: the benign auto-accept runs before the posture layer, so a proven-benign change proceeds under strict too). Fail-closed.
- ${gateInstallLine({ code: true })}
- Tiered ladder: tier-0 deterministic contract-diff is the live, deterministic leg and runs first. Above it the ladder is built as in-path seams - a cloud tier-1 corpus lookup (a contract judged once clears or condemns it everywhere), a tier-2 LLM consult on the ambiguous, and a tier-3 behavioral verifier that exercises a changed tool to clear or refute the change - but each is held off by default and requires explicit opt-in; the default build sends no arguments, results, server names or URLs and stays fail-closed; it does post a per-call receipt, disabled with MCPINDEX_RECEIPT_INGEST_ENABLED=0.
- Honest limits: contract_diff_not_safety_verdict (a HOLD means the contract CHANGED vs your pin, not that the new contract is unsafe; when enabled, the behavioral tier clears or refutes a change, it does not prove a tool safe); tiers1to3_held_off_by_default_opt_in; default_build_sends_no_args_results_or_names_receipt_optional; calibrated_false_v1 (confidence reported but not yet calibrated against a held-out corpus).
