...
function buildBody(servers: IndexedServer[], guides: Guide[]): string {
  ...
  const parts: string[] = [
    ...
    'Honest limits: contract_diff_not_safety_verdict (a HOLD means the contract CHANGED vs your pin, not that it is unsafe; when enabled, the behavioral tier clears or refutes, it does not prove a tool safe); tiers1to3_held_off_by_default_opt_in; default_build_sends_no_args_results_or_names_receipt_optional; calibrated_false_v1 (confidence reported but not yet calibrated against a held-out corpus).',
