...
function buildBody(servers: IndexedServer[]): string {
  ...
  const parts: string[] = [
    ...
    'Install: one-click config-wire across Claude Desktop / Claude Code / Cursor / Gemini CLI / Cline / Zed via uv tool install mcpindex-gate (or curl https://mcpindex.ai/install.sh | sh). Deprecated alias: mcpindex-preflight. Zero credentials change hands. Or the SDK wrap() one-liner (TS + Python). See https://mcpindex.ai/docs.',
