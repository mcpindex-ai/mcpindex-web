...
function coerceEvent(x: unknown): LedgerEvent | null {
  ...
  return {
    ...
    last_seen: typeof e.last_seen === 'string' ? e.last_seen : '',
  };
}
...
export async function loadLedger(): Promise<Ledger | null> {
  ...
  return {
    ...
    generated_at: typeof blob.generated_at === 'string' ? blob.generated_at : '',
    framing: typeof blob.framing === 'string' ? blob.framing : '',
