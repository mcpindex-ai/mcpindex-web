...
function posturesTwin(): string {
  ...
  * guard is the default posture.
  ...
  MONITOR releases every drift hold: every kind returns PROCEED-with-note. A tamper hold and
  an internal error sit above the posture layer and still stop the call.
   ...
   DEFAULT  | per-call receipt --------------------> [ mcpindex ingest ]
    ...
    OPT-IN  | HMAC fingerprint --------------> [ drift network ]
     ...
     NEVER  X tokens - arguments - results - schema bodies - your call data
            X (the hash and the fingerprint are over PUBLIC declarations:
            X  for a registry-listed server they match back to the name)`,
  },
  {
    id: 'tier-ladder',
    fig: '04',
