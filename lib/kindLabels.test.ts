...
test('KIND_LABEL covers the accepted taxonomy member-for-member (no unlabeled or orphan kinds)', () => {
  // A new taxonomy member without a label would ship a raw token to users; a label for a removed
  // member is dead weight. Enforce exact set equality over BOTH accepted sets so either drift
  // fails here.
  assert.deepEqual(
    Object.keys(KIND_LABEL).sort(),
    [...SURFACE_CHANGE_KINDS, ...CONTEXT_SURFACE_CHANGE_KINDS].sort(),
  );
});
