...
- [ ] R2-B2 Package name mismatch. Installer (install.sh) uses PKG=mcpindex-preflight + tooling.cse.proxy; buildable pyproject = mcpindex-trust packaging src/trust only. Site copy matches the INSTALLER (mcpindex-preflight) -> keep site copy, add PUBLISH-COUPLING note + a build-time link-check that site package name == installer PKG. Reconciliation (publish the wheel as mcpindex-preflight incl tooling.cse) is a product-repo go-live gate, noted out-of-band.
