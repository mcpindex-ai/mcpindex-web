...
1. **In-path drift gate** — one command wires Claude Desktop / Claude Code / Cursor / Gemini CLI / Cline / Zed so each MCP server launches behind the gate. Local, fail-closed. The default build sends no arguments, results, server names or URLs; it does post a per-call receipt, which MCPINDEX_RECEIPT_INGEST_ENABLED=0 turns off.  
